...
parser.add_option("--prot", action="store_true", dest="molProt", default=False,
                  help="reduce protein")

# --dna option: reduce dna
parser.add_option("--dna", action="store_true", dest="molDna", default=False,
                  help="reduce dna")

# --red option: correspondance between atoms and beads (coarse grain)
parser.add_option("--red", action="store", type="string", dest="redName",
                  help="correspondance file between atoms and beads")

# --ff option: coarse grain force field parameters
parser.add_option("--ff", action="store", type="string", dest="ffName",
                  help="force field parameters")

# --conv option: eventually, a correspondance between different names for the same atom
parser.add_option("--conv", action='store', type='string', dest='convName',
                  help='type conversion file')

parser.add_option("--allow_missing", action="store_true", dest="warning", default=False,
                  help="don't stop program if atoms are missing, only display a warning on stderr")
...
for atom in atomList:
    ...
    if resTag not in residueTagList:
        if resName in resBeadAtomModel:
