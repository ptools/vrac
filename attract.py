...
from ptools import (AttractForceField1, AttractForceField2,
                    AttractPairList, AttractRigidbody,
                    Coord3D,
                    Lbfgs,
                    Rigidbody,
                    Rmsd,
...
def readParams(filename):
    ...
    for i in range(nbminim):
        ...
        if int(line[-2]) == 1:
            ...
            minim['rstk'] = 0.0
        minimlist.append(minim)
    # return a list of type (iter,squarecutoff,has_restraint)
...
parser.add_option("-r", "--receptor", action="store", type="string", dest="receptor_name",
                  help="name of the receptor file")
parser.add_option("-l", "--ligand", action="store", type="string", dest="ligand_name",
...
parser.add_option("-t", "--translation", action="store", type="int", dest="transnb",
                  help="translation number (distributed mode) starting from 0 for the first one!")
parser.add_option("--start1", action="store_true", default=False, dest="start1",
                  help="(only useful with -t), use 1 for the first translation point")
...
transnb = 0
if options.transnb is not None:
    # check for rotation.dat and translation.dat
    checkFile("rotation.dat", "rotation file is required.")
...
if not options.single and printFiles:
